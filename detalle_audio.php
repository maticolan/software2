...
if (!empty($nombre)) {
    //FUB-18: obtener_detalle_audio(nombre_prod VARCHAR(255)) Consulta los detalles completos del producto de tipo audio (incluye información de promoción si existe).
    $stmt = $conn->prepare("
        ...
        WHERE c.nombre_prod = ? AND t.tipo = 'audio'
...
$ya_deseado = false;
if ($correo) {
    //FUB-19: verificar_producto_deseo(correo_usuario VARCHAR(255), nombre_prod VARCHAR(255)) Verifica si el producto ya está en la lista de deseos del usuario.
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    if (!$correo) {
        ...
        if ($_POST['action'] === 'add') {
            // PRO-: agregar_producto_deseo(correo_usuario VARCHAR(255), nombre_prod VARCHAR(255)) Agrega un producto a la lista de deseos del usuario.
            $stmt = $conn->prepare("INSERT INTO listadeseo (correo_usuario, nombre_prod) VALUES (?, ?)");
            $stmt->bind_param("ss", $correo, $nombre);
            $stmt->execute();
        } elseif ($_POST['action'] === 'remove') {
            //PRO-: eliminar_producto_deseo(correo_usuario VARCHAR(255), nombre_prod VARCHAR(255)) Elimina un producto de la lista de deseos del usuario.
            $stmt = $conn->prepare("DELETE FROM listadeseo WHERE correo_usuario = ? AND nombre_prod = ?");
            $stmt->bind_param("ss", $correo, $nombre);
            $stmt->execute();
        }
        header("Location: detalle_audio.php?nombre=" . urlencode($nombre));
        exit();
    }
}
...
$ya_en_carrito = false;
if ($correo) {
    //FUB-20: verificar_producto_carrito(correo_usuario VARCHAR(255), nombre_prod VARCHAR(255)) Verifica si el producto ya está en el carrito del usuario.
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['add_carrito']) && $correo) {
    if (!$ya_en_carrito) {
        //PRO-: agregar_producto_carrito(correo_usuario VARCHAR(255), nombre_prod VARCHAR(255)) Agrega un producto al carrito del usuario.
...
$todos_formatos = [];
//FUB-21: obtener_todos_los_formatos_audio() Lista general de todas las extensiones registradas como audio.
$stmt = $conn->prepare("SELECT extension FROM tipo_archivo WHERE tipo = 'audio'");
$stmt->execute();
...
$formatos = [];
//FUB-22: obtener_formatos_audio_disponibles(nombre_prod VARCHAR(255)) Consulta las extensiones disponibles de audio para el producto.
$stmt = $conn->prepare("
    SELECT ta.extension
    FROM tipo_archivo ta
    JOIN contenido c ON ta.id = c.tipo_archivo_id
    WHERE c.nombre_prod = ? AND ta.tipo = 'audio'
");
$stmt->bind_param("s", $nombre);
$stmt->execute();
$result = $stmt->get_result();
...
//FUB-22: Consultar los formatos de audio disponibles para el producto específico.
$stmt = $conn->prepare("
    SELECT ta.extension
    FROM tipo_archivo ta
    JOIN contenido c ON ta.id = c.tipo_archivo_id
...
<head>
    ...
    <style>
        * { box-sizing: border-box; font-family: Arial, sans-serif; margin: 0; padding: 0; }
        ...
        .detalle { display: flex; flex-wrap: wrap; justify-content: center; padding: 60px 80px; gap: 60px; }
        .preview { flex: 1; max-width: 450px; display: flex; flex-direction: column; align-items: center; gap: 20px; }
        ...
        .preview-modal audio { max-width: 90%; }
                .footer {
            background-color: #e0e0e0;
            padding: 40px;
            display: flex;
...
<div class="preview-modal" id="modalPreview" onclick="closePreview()">
    <audio controls autoplay src="<?= $ruta ?>"></audio>
</div>
