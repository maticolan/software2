...
/*
FUN-047: Obtener detalles del producto de tipo video
Consulta la base de datos para obtener los detalles del producto de tipo video según el ID recibido por GET.
Si no se encuentra, muestra un mensaje y redirige.
*/
if (!empty($nombre)) {

    //FUB-30: obtener_detalle_video(nombre_prod VARCHAR(255)) Consulta los detalles completos del producto de tipo video (incluye posible promoción).
    $stmt = $conn->prepare("
        ...
        WHERE c.nombre_prod = ? AND t.tipo = 'video'
    ");
...
/*
FUN-048: Verificar si el producto ya está en la lista de deseos
Consulta si el usuario ya tiene este producto en su lista de deseos.
*/
$ya_deseado = false;
if ($correo) {
    //FUB-31: verificar_producto_deseo_video(correo_usuario VARCHAR(255), nombre_prod VARCHAR(255)) Verifica si el producto de tipo video ya está en la lista de deseos del usuario.
...
/*
FUN-049: Añadir o quitar producto de la lista de deseos
Si se recibe una petición POST, añade o elimina el producto de la lista de deseos según la acción seleccionada.
*/
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    if (!$correo) {
        ...
        if ($_POST['action'] === 'add') {
            //PRO-39: agregar_producto_deseo_video(correo_usuario VARCHAR(255), nombre_prod VARCHAR(255)) Agrega un producto de tipo video a la lista de deseos del usuario.
            $stmt = $conn->prepare("INSERT INTO listadeseo (correo_usuario, nombre_prod) VALUES (?, ?)");
            $stmt->bind_param("ss", $correo, $nombre);
            $stmt->execute();
        } elseif ($_POST['action'] === 'remove') {
            //PRO-40: eliminar_producto_deseo_video(correo_usuario VARCHAR(255), nombre_prod VARCHAR(255)) Elimina un producto de tipo video de la lista de deseos del usuario.
            $stmt = $conn->prepare("DELETE FROM listadeseo WHERE correo_usuario = ? AND nombre_prod = ?");
            $stmt->bind_param("ss", $correo, $nombre);
            $stmt->execute();
        }
    header("Location: detalle_video.php?nombre=" . urlencode($nombre));
        exit();
    }
}

/*
FUN-050: Verificar si el producto ya está en el carrito
Consulta si el usuario ya tiene este producto en su carrito.
*/
$ya_en_carrito = false;
if ($correo) {
    //FUB-32: verificar_producto_carrito_video(correo_usuario VARCHAR(255), nombre_prod VARCHAR(255)) Verifica si el producto de tipo video ya está en el carrito del usuario.
...
/*
FUN-051: Añadir producto al carrito
Si se recibe una petición POST para añadir al carrito y el producto no está ya en el carrito, lo inserta en la tabla correspondiente.
*/
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['add_carrito']) && $correo) {
    if (!$ya_en_carrito) {
        //PRO-41: agregar_producto_carrito_video(correo_usuario VARCHAR(255), nombre_prod VARCHAR(255)) Agrega un producto de tipo video al carrito del usuario.
...
$todos_formatos = [];
//FUB-33: obtener_todos_los_formatos_video() Lista general de extensiones registradas como video.
$stmt = $conn->prepare("SELECT extension FROM tipo_archivo WHERE tipo = 'video'");
$stmt->execute();
...
/*
FUN-052: Obtener extensiones permitidas para videos
Consulta la base de datos para obtener los formatos (extensiones) de video disponibles y los almacena en un arreglo.
*/
$formatos = [];
//FUB-34: obtener_formatos_video_disponibles(nombre_prod VARCHAR(255)) Consulta las extensiones disponibles de video para el producto específico.
$stmt = $conn->prepare("
    SELECT ta.extension
...
$stmt = $conn->prepare("
    ...
    FROM tipo_archivo ta
    JOIN contenido c ON ta.id = c.tipo_archivo_id
    WHERE c.nombre_prod = ? AND ta.tipo = 'video'
");
$stmt->bind_param("s", $nombre);
$stmt->execute();
$result = $stmt->get_result();
...
<head>
    ...
    <style>
        * { box-sizing: border-box; font-family: Arial, sans-serif; margin: 0; padding: 0; }
        ...
        .detalle { display: flex; flex-wrap: wrap; justify-content: center; align-items: flex-start; padding: 60px 80px; gap: 80px; }
        .preview { flex: 1; max-width: 450px; display: flex; justify-content: flex-end; }
        .preview img { width: 100%; max-width: 420px; border-radius: 10px; box-shadow: 0 0 15px rgba(0,0,0,0.15); cursor: pointer; }
        .info { flex: 1; max-width: 500px; display: flex; flex-direction: column; gap: 20px; }
        h2 { margin: 0; font-size: 26px; }
        .btn-carrito { padding: 10px 20px; background: #6a0dad; color: white; border: none; border-radius: 5px; cursor: pointer; }
        .btn-disabled { background: #999 !important; cursor: not-allowed; }
        .corazon { font-size: 24px; color: gray; cursor: pointer; }
        ...
        .preview-modal { display: none; position: fixed; top: 0; left: 0; width: 100vw; height: 100vh; background: rgba(0,0,0,0.8); justify-content: center; align-items: center; z-index: 1000; }
        .preview-modal video { max-width: 90%; max-height: 90%; }
    </style>
    <script>
...
<div class="preview-modal" id="modalPreview" onclick="closePreview()">
    <video controls autoplay src="<?= $ruta ?>"></video>
</div>
